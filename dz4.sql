...
VALUES 
 ...
 SELECT c. `id`, c.`name`,sh.`shopname` 
 FROM `cats` c 
 LEFT JOIN `shops` sh 
 ON c.shops_id = sh.`id`;
...
SELECT c. `id`, c.`name`,sh.`shopname`
FROM (SELECT * FROM `cats` WHERE `cats`.`name` = "Murzik") c 
LEFT JOIN `shops` sh 
ON c.shops_id = sh.`id`;

-- 3. Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT c. `id`, c.`name`, c.shops_id, sh.id, sh.`shopname`
FROM (SELECT * FROM `cats` WHERE `cats`.`name` = "Murzik" OR `cats`.`name` = "Zuza") c
LEFT JOIN `shops` sh 
ON c.shops_id != sh.`id` ;
...
FROM Analysis a
RIGHT JOIN 
(SELECT o.ord_datetime, o.ord_an   
FROM Orders o
WHERE o.ord_datetime >= '2020-02-05 00:00:00' AND  o.ord_datetime<='2020-02-12 23:59:59' ) o
ON a.an_id = o.ord_an;
